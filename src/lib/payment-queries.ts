...
/* ═══════════════════════════════════════════════════════════════
 *  Recalculate Guards — 3-tier safety system
 * ═══════════════════════════════════════════════════════════════ */

export type RecalcGuardResult =
  | { guard: "credit_memo"; blocked: true; message: string }
  | { guard: "paid_rows"; blocked: true; message: string }
  | { guard: "manual_correction"; blocked: false; diverged: true; existing: { due_date: string; amount_due: number }[]; proposed: { due_date: string; amount_due: number }[] }
  | { guard: "clean"; blocked: false; diverged: false };

/**
 * Run all 3 recalculate guards and return the first that triggers.
 * Guard 1: credit memo hard block
 * Guard 2: paid-row hard block
 * Guard 3: manual-correction detection (divergence from engine output)
 */
export async function runRecalcGuards(
  invoiceId: string,
  invoiceDate: string,
  total: number,
  vendor: string,
  ...
  const { data: rows, error } = await supabase
    .from("invoice_payments")
    .select("id, payment_status, terms, amount_paid, amount_due, due_date, is_paid, installment_label")
    .eq("invoice_id", invoiceId)
    .order("due_date");
  ...
  // ── Guard 1: Credit memo hard block ──
  const hasCreditMemo = existing.some(
    r => r.terms === "credit_memo" || r.installment_label === "Credit" || (r.amount_due ?? 0) < 0
  );
  if (hasCreditMemo) {
    return {
      guard: "credit_memo",
      blocked: true,
      message: "This invoice contains credit memo payment rows. Credit memos are one-shot records and cannot be regenerated from payment terms.\n\nIf you believe this credit is incorrect, void the invoice and re-upload the credit PDF.",
    };
  }

  // ── Guard 2: Paid-row hard block ──
  const hasPaidRows = existing.some(
    r => r.payment_status === "paid" || r.is_paid === true || (r.amount_paid ?? 0) > 0
  );
  if (hasPaidRows) {
    return {
      guard: "paid_rows",
      blocked: true,
      message: "This invoice has one or more payments already marked as paid. Recalculating would destroy payment history (paid dates, amounts, partial payments).\n\nIf the schedule is wrong and needs correction, void and re-enter this invoice, or manually edit individual unpaid rows through the database.",
    };
  }
  ...
  const proposedSummary = proposedInstallments.map(inst => ({ due_date: inst.due_date, amount_due: inst.amount_due }));

  // Compare
  const matches =
    existingSummary.length === proposedSummary.length &&
    existingSummary.every((e, i) => {
      const p = proposedSummary[i];
      return e.due_date === p.due_date && Math.abs(e.amount_due - p.amount_due) < 0.02;
    });

  if (!matches && existingSummary.length > 0) {
    return {
      guard: "manual_correction",
      blocked: false,
      diverged: true,
  ...
  return { guard: "clean", blocked: false, diverged: false };
}
 ...
 *
 * IMPORTANT: Callers must run runRecalcGuards() first and handle blocked results.
 * This function does NOT re-check guards — it trusts the caller.
 * Set auditOverride=true to write an audit log entry for manual-correction overrides.
 */
export async function recalculatePaymentsForInvoice(
  ...
  // Write audit log for manual-correction override
  if (auditOverride) {
    await supabase.from("recalc_audit_log" as any).insert({
      invoice_id: invoiceId,
      invoice_number: invoiceNumber,
      vendor,
      action: "recalculate_payments_with_manual_override",
      old_values: oldSnapshot,
      new_values: rows,
      metadata: { reason: "manual corrections overwritten", payment_terms: paymentTermsText },
      performed_by: "josh",
    });
  }
